...
export default function ClicksOverTimeChart({ byHour }) {
  ...
  const yTickValues = [
    ...new Set(
      [0, 1, 2, 3, 4].map((i) => Math.round((max * i) / 4))
    )
  ].sort((a, b) => a - b)
    ...
    entries.length <= maxXLabels
      ...
      : [
          ...
          ...Array.from({ length: maxXLabels - 2 }, (_, i) => {
            const idx = Math.round(((i + 1) / (maxXLabels - 1)) * (entries.length - 1))
            return entries[idx]
          }),
  ...
  const axisLabels = rawAxisLabels.filter((e) => {
    const dateKey = new Date(e[0]).toLocaleDateString(undefined, { month: "short", day: "numeric" })
  ...
  return (
    <div className="w-full overflow-x-auto" role="img" aria-label="Clicks over time chart">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto min-h-[180px]" preserveAspectRatio="xMidYMid meet" aria-hidden>
        {axisLabels.map(([t], i) => {
          const x = padding.left + (innerWidth * (t - minT)) / rangeT
        ...
        {yTickValues.map((val) => {
          ...
          return (
            <text key={val} x={padding.left - 6} y={y + 4} className="fill-gecko-slate text-[10px]" textAnchor="end">
              {val}
            </text>
          )
        })}
