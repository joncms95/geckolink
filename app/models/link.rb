...
class Link < ApplicationRecord
  ...
  validates :target_url, presence: true, format: { with: %r{\Ahttps?://[^\s]+\z}i, message: "must be a http or https URL" }
  ...
  def target_url_must_have_valid_host
    ...
    host = URI.parse(target_url).host.to_s

    unless host.match?(VALID_HOST_REGEX)
