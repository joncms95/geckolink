...
class TitleFetcherJob < ApplicationJob
  ...
  # Browser User-Agent so sites like YouTube/Facebook don't block or return bot-only pages
  ...
  def perform(link_id)
    ...
    updates[:icon_url] = icon_url if icon_url.present?
  ...
  def extract_title(html)
    ...
    # Fallback: many sites (e.g. Facebook, shared pages) use Open Graph or Twitter meta tags
  ...
  def resolve_icon_url(html, page_uri)
    ...
    # 2. Try <link rel="icon"> (and apple-touch-icon)
  ...
  def icon_from_manifest(manifest_uri, base)
    ...
    # Prefer "any" purpose, then pick best size (192 or 96 or first)
  ...
  def extract_favicon_from_html(html, base)
    doc = Nokogiri::HTML(html)
    # Prefer <link rel="icon"> then apple-touch-icon
